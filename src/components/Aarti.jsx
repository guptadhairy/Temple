...
const Aarti = () => {
    ...
    return (
        <Stack direction={['column', 'row']} h={'105vh'}>
            <VStack w={'full'}>
                <video controls controlsList='nodownload' style={{ width: '100%' }} src={videoSrc}></video>
                {/* This is for youtube vdos */}
                {/* <iframe height={'70%'} width={'100%'} src={videoSrc} ></iframe> */}

                <VStack alignItems={'flex-start'} p={8} width={'full'} overflowY={'auto'}>
                    <Heading>Aarti Videos </Heading>
                    <Text>Hari Om Tat Sat . These are the video recordings of Shri Rameshwar Dham Mandir. Feel the energy and ultimate vibes of the beutiful Dham.</Text>
                </VStack>
            </VStack>
            <VStack w={['full','xl']} alignItems={'stretch'} p={8} spacing={8} overflowY={'auto'}>
                {
                    videosArr.map((item,index)=>(
                        <Button  onClick={()=>setVideoSrc(item)}>Aarti Video : {index +1}</Button>
                    ))
                }
            </VStack>
        </Stack>
